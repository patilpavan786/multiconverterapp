...
import style from "./AreaConvert.module.css"
import CompareArrowsIcon from "@mui/icons-material/CompareArrows";
import ArrowBackIcon from '@mui/icons-material/ArrowBack';
import { useNavigate } from 'react-router-dom';
function HectorToacre() {
  ...
  {toggle?     <div  className={style.container1}>
        <h1>hector</h1>
      <input placeholder={`${((value2) / 2.4710538147).toFixed(6)}  hector`}
        onChange={(e) => setValue1(e.target.value)} className={style.input} />
      </div  >:<div className={style.container1}>
      <h1> acres</h1>
      <input placeholder={`${((value1) * 2.4710538147).toFixed(6)}  acres`}
